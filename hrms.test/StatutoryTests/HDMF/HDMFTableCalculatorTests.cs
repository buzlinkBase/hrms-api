...
public class HDMFTableCalculatorTests : TestContextBase
{
    ...
    {
        // See SSSTableCalculatorTests' equivalent test — Variable's bracket lookup is already
        // scoped to actual gross earned so far this period, so it's withheld in full rather
        // than divided again (unlike Fixed, whose monthly-rate projection genuinely needs
        // splitting by the cutoff count).
        ...
        var result2 = new DeductionPipeline().Run(cutoff2);
        result2.HDMF.EE.Should().Be(225); // 675 - 450
    ...
    public void Weekly_Variable_BracketsEachWeekOffActualGrossToDate_TruesUpOnLastWeek()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            SeedHDMFTable(ctx, Brackets);
            if (priorGross > 0) AddPriorPayroll(ctx, priorGross);
            if (postedEE > 0) AddHDMFContribution(ctx, postedEE, 0);

            var result = new DeductionPipeline().Run(ctx);
            result.HDMF.EE.Should().Be(expected[i], because: $"week starting Feb {starts[i]}");

            postedEE += result.HDMF.EE;
            priorGross += weekGross;
            totalEE += result.HDMF.EE;
        }

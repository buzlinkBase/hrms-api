...
{
    public partial class OTMasterRecord : DevExpress.XtraEditors.XtraForm
    {
        public OTMasterRecord()
        {
            InitializeComponent();
        }
        ...
        void ViewRecord()
        {
            try
            {
                ...
                {
                    var data= new OverTimeController(command).FindRange(dateRangeFilter1.FromDate, dateRangeFilter1.ToDate, "%");
        ...
        {
            var model = gridView1.GetFocusedRow() as OverTimeApplicationEntity;
			if (model==null)
			{
                Msg.Inform("Select transaction first");
                return;
			}
            var frm = new CreateOT(model);
            frm.OnSaved += Frm_OnSaved; ;
            DisplayForms.ViewFormDialog(frm);
        }
        ...
        void Cancel()
        {
			try
			{
                ...
                using (var command = new UCommand())
                {
                    new OverTimeController(command).Cancel(model);
        ...
        void Approve()
        {
            try
            {
                ...
                using (var command = new UCommand())
                {
                    new OverTimeController(command).Approve(model);
					if (command.Commit()) ViewRecord();
                }
            }
            catch (Exception ex)

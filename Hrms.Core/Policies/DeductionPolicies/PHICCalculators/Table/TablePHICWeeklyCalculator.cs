...
{
    public class TablePHICWeeklyCalculator : IDeductionCalculator
    {
        public DeductionPipeData Calculate(DeductionPayloadContext context, DeductionPipeData line)
        {
            ...
            // earned so far this month, so table.EmployeeShare/EmployerShare here is already
            // the correct amount for income earned to date. Spreading it across the remaining
            // weeks -- or the cross-month/mid-hire day-proration below, which only makes
            // sense against a Fixed employee's whole-month projection -- would under-withhold
            // every non-final week and force a disproportionate true-up onto the last one
            // (see GetBalance's netting, which is what actually makes this correct).

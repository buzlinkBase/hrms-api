...
{
    public class TablePHICMonthlyCalculator : IDeductionCalculator
    {
        public DeductionPipeData Calculate(DeductionPayloadContext context, DeductionPipeData line)
        {
            ...
            var rate = PHICHelper.GetTable(context, baseRate);
            if (rate == null) return line;

            var balances = PHICHelper.GetBalance(context, rate.EmployeeShare, rate.EmployerShare);

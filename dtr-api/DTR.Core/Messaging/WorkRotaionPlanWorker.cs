...
public class  WorkRotaionPlanWorker : BackgroundService
{
    private readonly KafkaSettings _settings;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ConsumerConfig _config;

    public WorkRotaionPlanWorker(IServiceScopeFactory scopeFactory,
    ...
    private async Task ProcessKafkaMessages(CancellationToken stoppingToken)
    {
        using var consumer = new ConsumerBuilder<string, string>(_config)
            ...
            .Build();
        consumer.Subscribe(_settings.Topics.WorkRotationPlan);
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    var model = ObjectSerializer.Deserialize<MessagePayload<List<WorkSchedulePlanPayload>>>(result.Message.Value);
                    if (model?.Data == null)
                    {
                        Log.Warning($"Poison Pill detected: {nameof(ChangedHolidayWorker)}: Skipping.", result.TopicPartitionOffset);
                        consumer.Commit(result);
                        continue;
                    }
                    using var scope = _scopeFactory.CreateScope();
                    var mapper = scope.ServiceProvider.GetRequiredService<IMapper>();
                    var service = scope.ServiceProvider.GetRequiredService<WorkRotationPlanService>();
                    var entityModels  = mapper.Map<List<WorkSchedulePlan>>(model);
                    service.AddRange(entityModels);

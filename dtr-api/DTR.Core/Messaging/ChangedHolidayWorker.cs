...
public class  ChangedHolidayWorker : BackgroundService
{
    private readonly KafkaSettings _settings;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ConsumerConfig _config;

    public ChangedHolidayWorker(IServiceScopeFactory scopeFactory,
    ...
    private async Task ProcessKafkaMessages(CancellationToken stoppingToken)
    {
        using var consumer = new ConsumerBuilder<string, string>(_config)
            ...
            .Build();
        consumer.Subscribe(_settings.Topics.ChangeHoliday);
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    var model = ObjectSerializer.Deserialize<MessagePayload<ChangeHolidayPaload>>(result.Message.Value);
                    if (model?.Data == null)
                    {
                    ...
                    var entityModel  = mapper.Map<ChangeHoliday>(model);

...
public class EmployeeWorker : BackgroundService
{
    private readonly KafkaSettings _settings;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ConsumerConfig _config;

    public EmployeeWorker(IServiceScopeFactory scopeFactory,
    ...
    private async Task ProcessKafkaMessages(CancellationToken stoppingToken)
    {
        using var consumer = new ConsumerBuilder<string, string>(_config)
            ...
            .Build();
        consumer.Subscribe(_settings.Topics.Employee);
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    var model = ObjectSerializer.Deserialize<MessagePayload<EmployeePayload>>(result.Message.Value);
                    if (model?.Data == null)
                    {
                        Log.Warning($"Poison Pill detected: {nameof(EmployeeWorker)}: Skipping.", result.TopicPartitionOffset);
                    ...
                    var employee = mapper.Map<Employee>(model.Data);
                    empService.AddOrUpdate(employee);
                    await empService.SaveChangesAsync(stoppingToken);
                    await empService.CommitChangesAsync(stoppingToken);
                    consumer.Commit(result);
